...
void execute(char ***ligne_commande, bool background, int nb_commande, bool *leave, int *pid_actif, char *in, char *out)
{
    ...
    *pid_actif = 0;
    ...
    t_pipe pipes[nb_commande];
    for (int i = 0; i < nb_commande - 2; i++)
    {
        pipe(pipes[i].cote);
    }
    ...
    while (ligne_commande[indice_courant] != NULL)
    {
        ...
        if (!commande_interne)
        {
            ...
            extern_cmd(ligne_commande[indice_courant], pipes, background, pid_actif, in, out, indice_courant, nb_commande);
        }
...

{
    ...
    if (retour == 0)
    {
        ...
        if (nb_commande > 1)
        {
            ...
            /* Cas particulier premier processus */
            if (indice_courant == 0)
            {

                close(pipes[indice_courant].cote[1]);
                dup2(pipes[indice_courant].cote[0], STDOUT_FILENO);
            }

            else if (indice_courant == nb_commande - 2)
            {
                dup2(pipes[indice_courant].cote[0], STDIN_FILENO);
                close(pipes[indice_courant].cote[1]);
            }

            else if (indice_courant == nb_commande - 1)
            {
            }
            ...
            else
            {
                ...
                dup2(pipes[indice_courant - 1].cote[0], STDIN_FILENO);
                close(pipes[indice_courant - 1].cote[1]);

                dup2(pipes[indice_courant].cote[1], STDOUT_FILENO);
                close(pipes[indice_courant].cote[0]);
            }
        }
    ...
    else
    {
        ...
        /* Fermer les pipes des fils */
        for (int i = 0; i < nb_commande; i++)
        {
            close(pipes[i].cote[0]);
            close(pipes[i].cote[1]);
        }
